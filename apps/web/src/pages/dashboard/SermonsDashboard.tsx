...
export default function SermonsDashboard() {
  ...
  const getVimeoEmbed = (url: string) => {
    ...
    const match = url.match(regExp);
    return match
      ? `https://player.vimeo.com/video/${match[1]}?autoplay=0&muted=0&api=1&background=0`
      : url;
  };
  ...
  const stopMedia = (
    el: HTMLIFrameElement | HTMLVideoElement | HTMLAudioElement | null,
  ) => {
    if (!el) return;
    if (el instanceof HTMLVideoElement || el instanceof HTMLAudioElement)
      el.pause();
    else if (el instanceof HTMLIFrameElement) el.src = el.src;
  };
  ...
  if (sermons.length === 0)
    return <p className="text-center mt-10">Aucune prédication disponible.</p>;
              ...
              <div className="flex-1 bg-white rounded-xl shadow-lg p-4 transform scale-100 transition-all duration-700">
                ...
                {currentSermon.type === "VIDEO" ? (
                  isYouTube(currentSermon.url) ? (
                    <iframe
                      ref={(el) => {
                        mediaRefs.current[currentSermon.id] = el;
                      }}
